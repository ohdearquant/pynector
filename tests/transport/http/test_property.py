...
urls = st.text(min_size=1, max_size=100).filter(lambda x: not any(c.isspace() for c in x))
header_names = st.text(min_size=1, max_size=20, alphabet=st.characters(whitelist_categories=('Ll',)))
...
json_values = st.recursive(
    ...
    lambda children: st.lists(children, max_size=3) | st.dictionaries(st.text(min_size=1, max_size=5), children, max_size=3),
    max_leaves=10
)
...
class TestHttpMessageProperties:
    ...
    def test_headers_merge(self, headers1, headers2):
        ...
        # Create a message with headers1
        message = HttpMessage(headers=headers1)
...
class TestHTTPTransportFactoryProperties:
    ...
    def test_factory_creates_consistent_transport(self, base_url, headers, timeout, max_retries, retry_backoff_factor):
