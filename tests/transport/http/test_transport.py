...
from pynector.transport.errors import ConnectionError, ConnectionTimeoutError, MessageError
...
class TestHTTPTransport:
    ...
    async def test_connect_timeout(self):
        ...
        with patch("httpx.AsyncClient", side_effect=httpx.ConnectTimeout("Connection timeout")):
            transport = HTTPTransport()
            with pytest.raises(ConnectionTimeoutError, match="Connection attempt timed out"):
                await transport.connect()
    ...
    async def test_send_retry_success(self):
        ...
        with patch.object(httpx.AsyncClient, "request", side_effect=[mock_response_fail, mock_response_success]) as mock_request:
            await transport.connect()
            await transport.send(message)
    ...
    async def test_send_network_error_retry(self):
        ...
        with patch.object(httpx.AsyncClient, "request", side_effect=[httpx.ConnectError("Connection error"), mock_response]) as mock_request:
            await transport.connect()
            await transport.send(message)
    ...
    async def test_stream_response(self):
        ...
        with patch.object(httpx.AsyncClient, "stream", return_value=mock_response) as mock_stream:
            await transport.connect()
