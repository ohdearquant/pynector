...
def test_configure_exporters_with_otel():
    ...
    with patch('src.pynector.telemetry.config.HAS_OPENTELEMETRY', True):
        ...
        with patch('src.pynector.telemetry.config.BatchSpanProcessor', return_value=mock_batch_processor), \
             patch('src.pynector.telemetry.config.OTLPSpanExporter', return_value=mock_otlp_exporter), \
...
def test_configure_exporters_with_console():
    ...
    with patch('src.pynector.telemetry.config.HAS_OPENTELEMETRY', True):
        ...
        # Create the necessary imports and patch the actual implementation
        with patch('src.pynector.telemetry.config.BatchSpanProcessor', return_value=mock_batch_processor), \
             patch('src.pynector.telemetry.config.ConsoleSpanExporter', return_value=mock_console_exporter), \
...
def test_dummy_classes_more_coverage():
    ...
    if not HAS_OPENTELEMETRY:
        from src.pynector.telemetry.config import BatchSpanProcessor, OTLPSpanExporter, ConsoleSpanExporter
        
        # Test BatchSpanProcessor with specific options
        span_processor = BatchSpanProcessor("exporter", max_queue_size=100, schedule_delay_millis=5000)
