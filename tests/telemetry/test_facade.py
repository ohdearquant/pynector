...
def test_tracing_facade_init(has_opentelemetry):
    ...
    with patch('src.pynector.telemetry.facade.HAS_OPENTELEMETRY', has_opentelemetry):
        if has_opentelemetry:
            ...
            with patch('src.pynector.telemetry.facade.trace.get_tracer', mock_get_tracer):
    ...
    with patch('src.pynector.telemetry.facade.HAS_OPENTELEMETRY', has_opentelemetry):
        if has_opentelemetry:
            ...
            with patch('src.pynector.telemetry.facade.trace.get_tracer', mock_get_tracer):
                ...
                async with tracer.start_async_span("test_span", {"key": "value"}) as span_wrapper:
    ...
    with patch('src.pynector.telemetry.facade.HAS_OPENTELEMETRY', has_opentelemetry):
        if has_opentelemetry:
            ...
            with patch('src.pynector.telemetry.facade.trace.get_tracer', mock_get_tracer), \
                ...
                async with tracer.start_as_current_async_span("test_span", {"key": "value"}) as span_wrapper:
                    assert isinstance(span_wrapper, AsyncSpanWrapper)
                    assert span_wrapper.span == mock_span
            ...
            async with tracer.start_as_current_async_span("test_span", {"key": "value"}) as span:
                assert isinstance(span, NoOpSpan)
                assert span.name == "test_span"
                assert span.attributes == {"key": "value"}
...
def test_logging_facade_init(has_structlog):
    ...
    with patch('src.pynector.telemetry.facade.HAS_STRUCTLOG', has_structlog):
        if has_structlog:
            ...
            with patch('src.pynector.telemetry.facade.structlog.get_logger', mock_get_logger):
...
@pytest.mark.parametrize("has_structlog,has_opentelemetry", [
    (True, True),
    (True, False),
    (False, True),
    (False, False)
        ...
        if has_structlog:
            ...
            with patch('src.pynector.telemetry.facade.structlog.get_logger', mock_get_logger):
                if has_opentelemetry:
                    ...
                    with patch('src.pynector.telemetry.facade.trace.get_current_span', mock_get_current_span):
                        from src.pynector.telemetry.facade import LoggingFacade
                        logger = LoggingFacade("test_logger")
