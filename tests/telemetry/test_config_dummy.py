...
def test_configure_exporters_with_zipkin():
    ...
    with patch('src.pynector.telemetry.config.HAS_OPENTELEMETRY', True):
        ...
        with patch('src.pynector.telemetry.config.BatchSpanProcessor', return_value=mock_batch_processor), \
             patch('src.pynector.telemetry.config.ZipkinExporter', return_value=mock_zipkin_exporter), \
...
def test_configure_exporters_with_multiple_exporters():
    ...
    with patch('src.pynector.telemetry.config.HAS_OPENTELEMETRY', True):
        ...
        with patch('src.pynector.telemetry.config.BatchSpanProcessor', return_value=mock_batch_processor), \
             ...
             patch.dict(os.environ, {"OTEL_TRACES_EXPORTER": "otlp,console"}, clear=True):
