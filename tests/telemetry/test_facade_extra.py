...
def test_logging_facade_methods_with_import_error():
    ...
    with patch('src.pynector.telemetry.facade.HAS_STRUCTLOG', True), \
        ...
        with patch('src.pynector.telemetry.facade.structlog.get_logger', mock_get_logger), \
             patch('src.pynector.telemetry.facade.trace.get_current_span', mock_get_current_span):
...
def test_logging_facade_error_methods_with_import_error():
    ...
    with patch('src.pynector.telemetry.facade.HAS_STRUCTLOG', True), \
        ...
        with patch('src.pynector.telemetry.facade.structlog.get_logger', mock_get_logger), \
             patch('src.pynector.telemetry.facade.trace.get_current_span', mock_get_current_span), \
             patch('src.pynector.telemetry.facade.trace.Status', MagicMock(side_effect=ImportError("Test import error"))):
